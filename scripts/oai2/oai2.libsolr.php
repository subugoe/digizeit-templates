...
class oailib {
    ...
    function getRecords(&$arr, &$arrResult) {
        ...
        for ($i = 0; $i < min($arrResult['hits'], $arr['maxresults']); $i++) {
            if (!($arrData = oailucene::fetch_assoc($res)))
                break;
            ...
            foreach($arr['set'] as $set) {
                if(!in_array($set, $arrResult['header'][$i]['setSpec'])) {
                    array_unshift($arrResult['header'][$i]['setSpec'], $set);
                }
            }
    ...
    function getRelation(&$arr) {
        ...
        if (oailucene::num_rows($res)) {
            ...
            if (trim($arrRes['CREATOR']))
                $strReturn .= trim($arrRes['CREATOR']) . ': ';
            if (trim($arrRes['TITLE']))
                $strReturn .= trim($arrRes['TITLE']) . '. ';
            if (trim($arrRes['PLACEPUBLISH']))
                $strReturn .= trim($arrRes['PLACEPUBLISH']) . ' ';
